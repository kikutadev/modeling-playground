...
export function createHero(){
  ...
  return {root,handWorld,shotHandWorld,update(body,forward,dt,combat=null){
    ...
    const release=Math.max(0,1-(body.time-body.releaseTime)/.55);
    ...
    root.quaternion.slerp(pose,1-Math.exp(-dt*13));
    ...
    else if(!body.grounded){targets[0].set(-.57,.3+release*.3,.1);targets[1].set(.54,.17,-.21);}
    ...
    for(let i=0;i<2;i++){
      const start=shoulders[i],pole=point(i===0?-.8:.8,.0,.1),{joint,end}=solveLimb(start,targets[i],pole,.39,.37);
    ...
    for(let i=0;i<2;i++){
      ...
      const {joint,end}=solveLimb(hip,foot,pole,.48,.46);
