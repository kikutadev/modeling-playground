...
function restart(){
  body.reset();combat.reset();combatView.reset();finished=false;failed=false;combatTaught=false;swingTaught=false;yaw=0;pitch=.13;checkpoint=0;elapsed=0;lastPreWebLaunchTime=-10;lastPreWebLaunchVelocityY=0;keys.clear();mouseSwing=false;resetTouchState();direction();updateRings();snapCamera();
...
function snapCamera(){cameraForward.copy(forward);chaseOffset.copy(cameraForward).multiplyScalar(-7.4).add(new T.Vector3(0,3.6,0));aimOffset.copy(cameraForward).multiplyScalar(4).add(new T.Vector3(0,1,0));camera.position.copy(body.position).add(chaseOffset);cameraTarget.copy(body.position).add(aimOffset);camera.lookAt(cameraTarget);}snapCamera();
...
function render(now){
  ...
  // Keep the avatar readable at traversal speed. Speed sensation comes mostly from world motion and
  // a restrained FOV change; simultaneously pulling far back and widening heavily made authored
  // shoulder/hip/leg motion collapse to a few pixels on screen.
  const cameraDistance=6.6+Math.min(speed*.028,1.5)+(recentRelease?.35:0);
  ...
  const targetFov=64+Math.min(9,speed*.16)+(recentRelease||recentZip?1:0);camera.fov=T.MathUtils.lerp(camera.fov,targetFov,1-Math.exp(-dt*3.4));camera.updateProjectionMatrix();
  ...
  $('#state').textContent=landingLabel??(recentZip?'WEB ZIP':body.time-body.dodgeTime<.42?'DODGE':body.anchor?(body.grounded?'WEB RUN':'WEB SWING'):body.wall?(body.velocity.y>1?'WALL RUN':'WALL CONTACT'):body.grounded?'ROOFTOP / STREET':body.velocity.y>2?'RISING':'FREE FALL');
...
if(e2eParams.has('e2e')){
  ...
  globalThis.__threadlineE2E={
    performZip(){return {ok:webZip(),state:readPlayState()};},
    advanceSimulation(steps=1){
    ...
    placeTetheredGround(){
      body.release();body.position.set(0,1.1,-18);body.velocity.set(0,0,-18);
      body.grounded=true;body.wall=null;body.landingType='roll';body.landingStart=body.time;body.landingUntil=body.time+.5;
      const desired=new T.Vector3(0,0,-1),target=chooseTraversalAnchor(desired);
      if(!target)throw new Error('No tethered-ground fixture anchor');
      body.attach(target,{preload:traversalTuning.attachPreload});
      yaw=0;pitch=.13;direction();snapCamera();
      return readPlayState();
    }
  };
