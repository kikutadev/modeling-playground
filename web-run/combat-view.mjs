...
export function createCombatView(scene,combat){
  ...
  const drones=combat.drones.map(()=>{
    ...
    const hull=new T.Mesh(new T.OctahedronGeometry(1.1,0),metal);hull.scale.set(1.2,.5,.8);hull.castShadow=true;root.add(hull);
    const eye=new T.Mesh(new T.SphereGeometry(.28,12,8),light.clone());eye.position.set(0,0,.8);root.add(eye);
    const rotors=[];
    for(const x of [-1.4,1.4])for(const z of [-.8,.8]){
      const arm=new T.Mesh(new T.BoxGeometry(1.5,.12,.12),metal);arm.position.set(x*.55,0,z);root.add(arm);
      const disc=new T.Mesh(new T.TorusGeometry(.63,.07,6,18),metal);disc.rotation.x=Math.PI/2;disc.position.set(x,0,z);root.add(disc);
      const blade=new T.Mesh(new T.BoxGeometry(1.18,.04,.12),light);blade.position.copy(disc.position);root.add(blade);rotors.push(blade);
    }
    const cage=new T.LineSegments(new T.EdgesGeometry(new T.IcosahedronGeometry(1.65,1)),new T.LineBasicMaterial({color:0xcafff4,transparent:true,opacity:.85}));root.add(cage);
    ...
    const health=new T.Group();root.add(health);health.position.y=1.5;
    for(let i=0;i<3;i++){const pip=new T.Mesh(new T.BoxGeometry(.32,.1,.1),light);pip.position.x=(i-1)*.43;health.add(pip);}
  ...
  return {
    ...
    update(dt,body){
      drones.forEach((v,i)=>{
        ...
        v.root.rotation.z=Math.sin(combat.time*3+i)*.08;
      ...
      projectiles.forEach((mesh,i)=>{const p=combat.projectiles[i];mesh.visible=!!p;if(!p)return;mesh.position.copy(p.position);mesh.material=p.kind==='web'?webMat:boltMat;mesh.scale.setScalar(p.kind==='web'?.18:.32);});
      for(let i=flashes.length-1;i>=0;i--){const f=flashes[i];f.age+=dt;f.mesh.scale.setScalar(1+f.age*9);f.mesh.rotation.y+=dt*5;f.mesh.material.opacity=Math.max(0,1-f.age/f.duration);if(f.age>f.duration){scene.remove(f.mesh);f.mesh.geometry.dispose();f.mesh.material.dispose();flashes.splice(i,1);}}
    },
