...
export const DRONE_TARGET_RANGE=96;
export const DRONE_KICK_RANGE=28;
export const DRONE_KICK_CONTACT=5.4;
export const DRONE_HIT_RADIUS=3.5;
...
export class AirCombat {
  ...
  shoot(body,forward,origin=null){
    ...
    const goal=target?.position.clone()??start.clone().addScaledVector(forward,88);
    this.aimPoint=goal.clone();
    this.projectiles.push({kind:'web',position:start,velocity:goal.clone().sub(start).normalize().multiplyScalar(96),life:1.2,targetId:target?.id});
  ...
  kick(body,forward){
    ...
    body.release();body.velocity.copy(target.position).sub(body.position).normalize().multiplyScalar(46);
  ...
  step(dt,body){
    ...
    for(const d of this.drones){
      if(d.hp<=0)continue;
      if(d.stunUntil<this.time)d.position.copy(d.home).add(new Vector3(Math.sin(this.time*.6+d.id)*4.2,Math.sin(this.time*.9+d.id)*2.2,Math.cos(this.time*.45+d.id)*1.4));
      if(d.stunUntil>this.time||body.position.distanceTo(d.position)>74||!this.visible(d.position,body.position)) {d.charge=0;d.chargeAim=null;continue;}
    ...
    for(const p of this.projectiles){
      ...
      if(p.kind==='web'&&target?.hp>0)p.velocity.copy(target.position).sub(p.position).normalize().multiplyScalar(96);
    ...
    if(this.kickTarget!==null){
      ...
      }else if(this.time-this.kickAt>.58)this.kickTarget=null;
    }
  }
