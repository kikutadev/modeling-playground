...
export const DRONE_SPAWNS=[[0,24,-73],[-5,29,-151],[4,32,-232]];
...
export class AirCombat {
  ...
  target(position,forward,range=76){
    ...
    for(const d of this.drones){
      ...
      if(distance>range||facing<.45||!this.visible(position,d.position))continue;
  ...
  shoot(body,forward,origin=null){
    ...
    const goal=target?.position.clone()??start.clone().addScaledVector(forward,70);
    this.aimPoint=goal.clone();
    this.projectiles.push({kind:'web',position:start,velocity:goal.clone().sub(start).normalize().multiplyScalar(90),life:1.1,targetId:target?.id});
    this.events.push({type:'shot',position:start.clone()});return true;
  }
  kick(body,forward){
    if(this.time-this.kickAt<.65)return false;
    const target=this.target(body.position,forward,21);
    if(!target)return false;
    body.release();body.velocity.copy(target.position).sub(body.position).normalize().multiplyScalar(43);
  ...
  step(dt,body){
    ...
    for(const d of this.drones){
      if(d.hp<=0)continue;
      if(d.stunUntil<this.time)d.position.copy(d.home).add(new Vector3(Math.sin(this.time*.6+d.id)*2.5,Math.sin(this.time*.9+d.id)*1.4,0));
      if(d.stunUntil>this.time||body.position.distanceTo(d.position)>58||!this.visible(d.position,body.position)) {d.charge=0;d.chargeAim=null;continue;}
      if(this.time>=d.fireAt){
        ...
        if(d.charge>=1.2){
          this.projectiles.push({kind:'bolt',position:d.position.clone(),velocity:d.chargeAim.clone().sub(d.position).normalize().multiplyScalar(38),life:2.5});
    ...
    for(const p of this.projectiles){
      ...
      if(p.kind==='web'&&target?.hp>0)p.velocity.copy(target.position).sub(p.position).normalize().multiplyScalar(90);
      ...
      if(p.kind==='web'){
        for(const d of this.drones){if(d.hp>0&&distanceToSegment(d.position,old,end)<1.5){this.hitDrone(d,1,'web');p.life=0;break;}}
